...
const Navbar = () => {
  const [isSidebarOpen, setIsSidebarOpen] = useState(false);
  const [isUserMenuOpen, setIsUserMenuOpen] = useState(false);
  const [searchQuery, setSearchQuery] = useState('');  // FIX 4: Added search state
  const userMenuRef = useRef(null);
  ...
  // FIX 4: Handle search functionality
  const handleSearch = (e) => {
    if (e.key === 'Enter' && searchQuery.trim()) {
      navigate(`/all-hotels?search=${searchQuery}`);
    }
  };
  ...
  return (
    <>
      <nav className="absolute top-0 left-0 w-full z-20 flex items-center justify-between px-6 md:px-10 py-6 text-white bg-gradient-to-b from-black/60 to-transparent">
        {/* FIX 3: Made logo clickable to go home */}
        ...
        <div className="hidden md:flex items-center gap-6">
          ...
          <div className="relative">
            <input 
              ...
              className="bg-white/20 border border-white/30 rounded-full px-4 py-1.5 focus:outline-none focus:bg-white/30 placeholder-white text-sm w-48" 
            />
            <Search 
              className="absolute right-3 top-2 w-4 h-4 text-white/70 cursor-pointer" 
              onClick={() => searchQuery.trim() && navigate(`/all-hotels?search=${searchQuery}`)}
            />
          </div>

          {/* FIX 2: Fixed User Menu with working navigation */}
          <div className="relative" ref={userMenuRef}>
            <button 
            ...
            {isUserMenuOpen && (
              <div className="absolute right-0 mt-3 w-48 bg-white rounded-xl shadow-xl py-2 z-50 text-gray-800 border border-gray-100 overflow-hidden">
                ...
                {/* FIX 2: Added onClick handlers */}
                <button 
                  onClick={() => {
                    navigate('/my-bookings');
                    setIsUserMenuOpen(false);
                  }}
                  className="w-full flex items-center gap-3 px-4 py-3 hover:bg-blue-50 text-sm font-medium transition-colors"
                >
                  <User size={18} className="text-blue-600" /> User Dashboard
                </button>
                <button 
          ...
          <div className="border-t pt-6 space-y-4">
            {user ? (
              <>
                ...
                <button 
                  onClick={() => signOut(() => navigate("/"))}
                  className="w-full flex items-center justify-center gap-2 border border-red-200 text-red-500 py-3 rounded-lg font-bold"
                >
